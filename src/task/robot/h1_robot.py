...
class H1Robot(LeggedRobot):
    ...
    def init(self, mj_model: mujoco.MjModel, mj_data: mujoco.MjData):
        """
    ...
    def setMotorsTorque(self, tauIn: List[float]):
        """
        Set motor torques in MuJoCo
        
        Args:
            tauIn: List of torque values for each joint
        """
        for i in range(self.jointNum):
            self.mj_data.ctrl[i] = tauIn[i]
